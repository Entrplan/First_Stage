...
function oauth2_3lo_test() {
  const transporter = nodemailer.createTransport({
    ...
    auth: {
      ...
      user: 'user@example.com',
      clientId:
        '000000000000-xxx0.apps.googleusercontent.com',
      clientSecret: 'XxxxxXXxX0xxxxxxxx0XXxX0',
      refreshToken:
        '1/XXxXxsss-xxxXXXXXxXxx0XXXxxXXx0x00xxx',
      accessToken: 'ya29.Xx_XX0xxxxx-xX0X0XxXXxXxXXXxX0x',
...
function oauth2_message_options_test() {
  const transporter = nodemailer.createTransport({
    ...
    auth: {
      type: 'OAuth2',
      clientId:
        '000000000000-xxx.apps.googleusercontent.com',
      clientSecret: 'XxxxxXXxX0xxxxxxxx0XXxX0',
    },
  });
...
function ses_test() {
  ...
  transporter.sendMail(
    options,
    (err, info: SESTransport.SentMessageInfo) => {
      if (!err) {
        console.log(info.envelope);
        console.log(info.messageId);
      }
...
function plugin_compile_test() {
  ...
  transporter.use('compile', (mail, callback) => {
    if (
      ...
      mail.data.text = mail.data.html.replace(
        /<[^>]*>/g,
        ' ',
      );
    }
...
function plugin_stream_test() {
  ...
  transformer._transform = function transform(
    chunk: Buffer,
    encoding,
    done,
  ) {
...
function plugin_transport_example_test() {
  ...
  class Transport
    implements nodemailer.Transport<TestTransportInfo>
  {
    name = 'minimal';
    version = '0.1.0';
...
function isAddress(
  ...
  return (
    (addressOrGroup as addressparser.Address).address !==
    undefined
  );
}
...
function isGroup(
  ...
  return (
    (addressOrGroup as addressparser.Group).group !==
    undefined
  );
}
...
function addressparser_test() {
  ...
  const results: addressparser.AddressOrGroup[] =
    addressparser(input);
  ...
  if (isAddress(firstResult)) {
    ...
    const group: addressparser.AddressOrGroup[] =
      firstResult.group;
    const name: string = firstResult.name;
  }
}
...
function xoauth2_test() {
  ...
  xoauth2.getToken(false, (err, accessToken) => {
    if (err) throw err;
    const token: string =
      xoauth2.buildXOAuth2Token(accessToken);
  });
}
